# Claude usage — how the coding agent was directed

This is the agent-usage documentation the brief requires (its `AGENTS.md`/
`CLAUDE.md` deliverable): how Claude Code (Anthropic's CLI agent) was directed
on this project — the workflow, the standing instructions, where the agent's
output was accepted, and where it was overridden. The repo-root `CLAUDE.md` is
the operational context file the agent itself loads; this document is the
narrative for a human reviewer.
...
5. **Post-issue work continued the same shape.** Later sessions (e.g. the SQLite
   session-metrics persistence and the AWS deployment plan) followed the same
   pattern: explore, surface the design decision and its rationale before
   implementing, land as scoped commits with both test suites green.
...
collisions after reconnect), and the session-metrics persistence design (the
agent identified that only the frontend sees both modes' timings, so capture
had to be client-reported).
...
- Local persistence scope: the human chose SQLite and local-first, with the
  AWS mapping written down (`docs/deployment-plan.md`) before any infra work.
